...
- [ ] Task 1 : Backend — ProfileController et routes (AC: #1, #2)
  - [ ] 1.1 Créer `app/Http/Controllers/Api/ProfileController.php`
  - [ ] 1.2 Implémenter `updateProfile(Request $request)` :
  ...
  - [ ] 1.3 Implémenter `updatePassword(Request $request)` :
  ...
  - [ ] 1.4 Créer les routes protégées dans `routes/api.php` :
    ```php
...
- [ ] Task 2 : Backend — Request validation classes (AC: #1, #2)
  - [ ] 2.1 Créer `app/Http/Requests/UpdateProfileRequest.php` :
    - Rules : name, email avec unique exception pour l'utilisateur courant
    - Messages FR personnalisés
  - [ ] 2.2 Créer `app/Http/Requests/UpdatePasswordRequest.php` :
    - Rules : current_password, password, password_confirmation
    - Messages FR personnalisés
    - Validation custom pour vérifier current_password
  - [ ] 2.3 Utiliser ces Request classes dans ProfileController

- [ ] Task 3 : Backend — UserResource pour réponses API (AC: #1)
  - [ ] 3.1 Créer `app/Http/Resources/UserResource.php` :
    - Transformer le modèle User en JSON : id, name, email, created_at
    - Exclure les champs sensibles (password, tokens)
  - [ ] 3.2 Utiliser `UserResource` dans AuthController et ProfileController pour standardiser les réponses

- [ ] Task 4 : Flutter — Profile remote source (AC: #1, #2)
  - [ ] 4.1 Créer `lib/features/profile/data/profile_remote_source.dart` :
...
- [ ] Task 5 : Flutter — ProfileRepository (AC: #1, #2)
  - [ ] 5.1 Créer `lib/features/profile/data/profile_repository.dart` :
...
- [ ] Task 6 : Flutter — ProfileCubit et states (AC: #1, #2)
  - [ ] 6.1 Créer `lib/features/profile/presentation/cubit/profile_state.dart` :
    - `ProfileInitial`, `ProfileLoading`, `ProfileUpdated(UserModel user)`, `ProfileError(String message)`
  - [ ] 6.2 Créer `lib/features/profile/presentation/cubit/profile_cubit.dart` :
...
- [ ] Task 7 : Flutter — Page de profil (AC: #1, #2)
  - [ ] 7.1 Créer `lib/features/profile/presentation/pages/profile_page.dart` :
    ...
    - Section "Modifier le profil" :
      ...
      - Loading indicator pendant `ProfileLoading`
      - Afficher SnackBar succès si `ProfileUpdated`
      - Afficher SnackBar erreur si `ProfileError`
    - Section "Modifier le mot de passe" :
      - Champs : current_password, new_password, confirm_password (obscured)
      - Bouton "Changer le mot de passe"
      - BlocListener : écoute ProfileCubit
      - Loading indicator pendant `ProfileLoading`
      - Afficher SnackBar succès si `ProfileUpdated`
      - Afficher SnackBar erreur si `ProfileError`
  - [ ] 7.2 Créer `lib/features/profile/presentation/widgets/profile_form.dart` pour la section modification profil
  - [ ] 7.3 Créer `lib/features/profile/presentation/widgets/password_form.dart` pour la section modification mot de passe

- [ ] Task 8 : Flutter — Routing et navigation (AC: #1, #2)
  - [ ] 8.1 Ajouter route `/profile` dans `lib/app/routes.dart` :
    - Route protégée (accessible seulement si `AuthAuthenticated`)
    - Navigation : depuis HomePage ou menu latéral
  - [ ] 8.2 Modifier `HomePage` pour ajouter un lien vers l'écran profil

- [ ] Task 9 : Flutter — Mise à jour de AuthCubit après modification profil (AC: #1)
  - [ ] 9.1 Ajouter méthode `updateUser(UserModel user)` dans `AuthCubit` :
    - Mettre à jour le state `AuthAuthenticated` avec le nouveau user
    - Notifier les listeners pour rafraîchir l'UI
  - [ ] 9.2 Appeler cette méthode depuis `ProfileCubit.updateProfile()` après succès

- [ ] Task 10 : Tests backend (AC: #1, #2)
  - [ ] 10.1 Créer `tests/Feature/Profile/UpdateProfileTest.php` :
  ...
  - [ ] 10.2 Créer `tests/Feature/Profile/UpdatePasswordTest.php` :
...
- [ ] Task 11 : Tests Flutter (AC: #1, #2)
  - [ ] 11.1 Créer `test/features/profile/data/profile_repository_test.dart` :
  ...
  - [ ] 11.2 Créer `test/features/profile/presentation/cubit/profile_cubit_test.dart` :
  ...
  - [ ] 11.3 Créer `test/features/profile/presentation/pages/profile_page_test.dart` :
...
- [ ] Task 12 : Validation finale (AC: #1, #2)
  - [ ] 12.1 Vérifier le flow complet de modification profil : formulaire → API → mise à jour → refresh UI
  - [ ] 12.2 Vérifier le flow complet de modification mot de passe : formulaire → validation current_password → mise à jour
  - [ ] 12.3 Vérifier les erreurs : ancien mot de passe incorrect, email déjà utilisé, validation
  - [ ] 12.4 Vérifier que l'UI reflète immédiatement les changements après modification profil
  - [ ] 12.5 Exécuter tous les tests : `cd backend-api && ./vendor/bin/sail artisan test`
  - [ ] 12.6 Exécuter tous les tests Flutter : `cd mobile-app && flutter test`
  - [ ] 12.7 Commit : `git add . && git commit -m "feat(profile): gestion du profil utilisateur avec modification nom/email/password"`
