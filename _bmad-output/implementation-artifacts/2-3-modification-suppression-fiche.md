...
- [ ] Task 9 : Adapter le formulaire d'édition pour le sélecteur contact (AC: #1)
  - [ ] 9.1 Remplacer les champs texte agent par `AgentSelectorWidget` dans `edit_property_page.dart`
  - [ ] 9.2 Pré-sélectionner le contact actuel si `contactId` est renseigné
  - [ ] 9.3 Permettre de changer l'agent ou de le retirer (nullable)
  - [ ] 9.4 Mettre à jour `PropertyCubit.updateProperty()` pour `contactId`
