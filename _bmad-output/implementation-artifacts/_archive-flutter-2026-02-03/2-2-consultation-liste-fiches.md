...
- [x] Task 9 : Adapter l'affichage détail pour contact lié (AC: #2)
  - [x] 9.1 Modifier `property_agent_section.dart` pour récupérer le contact via `contactId` si présent
  - [x] 9.2 Fallback : afficher les anciens champs `agentName/agentAgency/agentPhone` si `contactId` est null
  - [x] 9.3 Ajouter lien cliquable vers la fiche contact dans le carnet d'adresses
