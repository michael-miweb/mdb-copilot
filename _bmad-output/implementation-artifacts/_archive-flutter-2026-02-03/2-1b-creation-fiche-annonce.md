...
- [x] Task 10 : Ajouter `contactId` (FK nullable) dans PropertiesTable Drift et migration backend
  - [x] 10.1 Ajouter colonne `contactId` dans `properties_table.dart` (already done)
  - [x] 10.2 Créer migration Laravel `add_contact_id_to_properties_table` (already done)
  - [x] 10.3 Mettre à jour `PropertyModel` avec champ `contactId`
  - [x] 10.4 Conserver les anciens champs `agentName/agentAgency/agentPhone` pour rétrocompatibilité données existantes

- [x] Task 11 : Remplacer les champs agent par le sélecteur contact dans le formulaire (AC: #2, #2b)
  - [x] 11.1 Créer widget `agent_selector_widget.dart` — dropdown filtré agents + bouton "Créer nouveau"
  - [x] 11.2 Intégrer `AgentSelectorWidget` dans `property_form_fields.dart` en remplacement des 3 champs texte
  - [x] 11.3 Mettre à jour `CreatePropertyPanel` et `EditPropertyPanel` pour `contactId`

- [x] Task 12 : Tests migration contact (AC: #2, #2b)
  - [x] 12.1 Test repository : createProperty avec contactId (2 tests)
  - [x] 12.2 Test cubit : createProperty lie correctement le contact (1 test)
  - [ ] 12.3 Test widget : sélecteur affiche uniquement les agents immobiliers (deferred — requires widget test pump with RepositoryProvider mock)
  - [ ] 12.4 Test widget : bouton création inline fonctionne (deferred — requires widget test pump with RepositoryProvider mock)
  - [x] 12.5 Test backend : contact_id user scope validation (3 tests)
